...
<%
			...
			EventBean eBean = dao.getBoard(num);//글검색 
			
			String DBSubject= eBean.getSubject(); //글제목
			String DBContent =""; //검색한 글 내용을 저장할 용도의 변수
			
			// 검색한 글의 내용이 있다면 내용들 엔터키 처리
			if(eBean.getContent() != null){
				DBContent = eBean.getContent().replace("<br>", "\r\n");
			}
...
<!-- 게시판 -->
		<article>
			<h1>이벤트 수정페이지 </h1>
			<form action=eventUpdatePro.jsp?pageNum=<%=pageNum %> method="post" name="fr" enctype="multipart/form-data">
